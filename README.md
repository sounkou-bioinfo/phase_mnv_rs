...
`bcftools index`. Plain `.vcf` output and stdout redirection are not indexable;
do not write `phase_mnv_rs ... > out.vcf.gz` and expect a BGZF-indexable file.
Use `-o out.vcf.gz` instead. Use `--threads N` / `-@ N` for htslib/BGZF worker
threads.
