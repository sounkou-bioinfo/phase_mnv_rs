...
This guard performs a calibration prepass and therefore cannot be combined with
`--max-reads`. Known variant sites are otherwise not masked, so mismatch rates
should be interpreted as sequencing-error-plus-variation unless regions are
